...
describe("Controller", () => {
  let mockRunner: Runner<IndexerRunnerDef>;
  let mockGenerator: AsyncGenerator<IndexerEvent, void>;

  class MockRunner extends IndexerRunner<IndexerRunnerDef> {
    run = vi.fn(() => mockGenerator);
  }
  ...
  describe("start", () => {
    ...
    it("should call runner.run with point", async () => {
      const controller = new Controller({ runner: mockRunner });
      const point = { slot: 100, id: "abc123" };

      const opts = { fn: vi.fn(), point };
      await controller.start(opts);

      expect(mockRunner.run).toHaveBeenCalledWith(opts);
    });
    ...
    it("should use default fn from constructor if not provided in start", async () => {
      ...
      const controller = new Controller({ runner: mockRunner }, { fn: mockFn });
  ...
  describe("event processing", () => {
    ...
    it("should apply throttle on filtered events", async () => {
      ...
      try {
        ...
        // Advance timers to process all throttle delays
    ...
    it("should apply throttle on both filtered and processed events", async () => {
      ...
      try {
        ...
        const mockFilter = vi.fn().mockImplementation(async () => {
          filterCallCount++;
          return filterCallCount % 2 === 0; // Process even-numbered calls
        });
        ...
        expect(mockFilter).toHaveBeenCalledTimes(3);
        expect(mockFn).toHaveBeenCalledTimes(1); // Only event 2 processed
        assert(controller.state.status !== "idle");
        expect(controller.state.counters.filterCount).toBe(2); // Events 1 and 3
        expect(controller.state.counters.applyCount).toBe(1); // Event 2 only
      } finally {
        vi.useRealTimers();
      }
  ...
  describe("logger", () => {
    it("should call logger on controller events", async () => {
      ...
      const startedEvent = mockLogger.log.mock.calls.find(
